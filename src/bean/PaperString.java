package bean;

import java.sql.Time;
import java.sql.Timestamp;
import java.util.Date;
/**
 * 把题目封装成试卷的片段
 * @author chaizhi
 *
 */
public class PaperString {
	private int id;
	private Timestamp starttime;
	private Timestamp endtime;
	private String papername;
	private String class1;
	private Time time;
	private String xzt1qu;
	private String xzt1an;
	private String xzt1opA;
	private String xzt1opB;
	private String xzt1opC;
	private String xzt1opD;
	
	private String xzt2qu;
	private String xzt2an;
	private String xzt2opA;
	private String xzt2opB;
	private String xzt2opC;
	private String xzt2opD;
	
	private String xzt3qu;
	private String xzt3an;
	private String xzt3opA;
	private String xzt3opB;
	private String xzt3opC;
	private String xzt3opD;
	
	private String xzt4qu;
	private String xzt4an;
	private String xzt4opA;
	private String xzt4opB;
	private String xzt4opC;
	private String xzt4opD;
	
	private String xzt5qu;
	private String xzt5an;
	private String xzt5opA;
	private String xzt5opB;
	private String xzt5opC;
	private String xzt5opD;
	
	private String xzt6qu;
	private String xzt6an;
	private String xzt6opA;
	private String xzt6opB;
	private String xzt6opC;
	private String xzt6opD;
	
	private String xzt7qu;
	private String xzt7an;
	private String xzt7opA;
	private String xzt7opB;
	private String xzt7opC;
	private String xzt7opD;
	
	private String xzt8qu;
	private String xzt8an;
	private String xzt8opA;
	private String xzt8opB;
	private String xzt8opC;
	private String xzt8opD;
	
	private String xzt9qu;
	private String xzt9an;
	private String xzt9opA;
	private String xzt9opB;
	private String xzt9opC;
	private String xzt9opD;
	
	private String xzt10qu;
	private String xzt10an;
	private String xzt10opA;
	private String xzt10opB;
	private String xzt10opC;
	private String xzt10opD;
	
	private String tkt1qu;
	private String tkt1an;
	
	private String tkt2qu;
	private String tkt2an;
	
	private String tkt3qu;
	private String tkt3an;
	
	private String tkt4qu;
	private String tkt4an;
	
	private String tkt5qu;
	private String tkt5an;
	
	private String pdt1qu;
	private String pdt1an;
	
	private String pdt2qu;
	private String pdt2an;
	
	private String pdt3qu;
	private String pdt3an;
	
	private String jdt1qu;
	
	private String jdt2qu;


	private String bct1qu;
	private String bct2qu;
	public PaperString(Xzt xzt1,Xzt xzt2, Xzt xzt3, Xzt xzt4, Xzt xzt5, 
			Xzt xzt6, Xzt xzt7, Xzt xzt8, Xzt xzt9, Xzt xzt10,
			Tkt tkt1,Tkt tkt2,Tkt tkt3,Tkt tkt4,Tkt tkt5,
			Pdt pdt1,Pdt pdt2,Pdt pdt3,Jdt jdt1,Jdt jdt2,Bct bct1,Bct bct2) {

		 xzt1qu=xzt1.getQuestion();
	     xzt1an=xzt1.getAnswer();
         xzt1opA=xzt1.getOptionA();
		 xzt1opB=xzt1.getOptionB();
	     xzt1opC=xzt1.getOptionC();
		 xzt1opD=xzt1.getOptionD();
		
		 xzt2qu=xzt2.getQuestion();
	     xzt2an=xzt2.getAnswer();
         xzt2opA=xzt2.getOptionA();
		 xzt2opB=xzt2.getOptionB();
	     xzt2opC=xzt2.getOptionC();
		 xzt2opD=xzt2.getOptionD();
		 
		 xzt3qu=xzt3.getQuestion();
	     xzt3an=xzt3.getAnswer();
         xzt3opA=xzt3.getOptionA();
		 xzt3opB=xzt3.getOptionB();
	     xzt3opC=xzt3.getOptionC();
		 xzt3opD=xzt3.getOptionD();
		 
		 xzt4qu=xzt4.getQuestion();
	     xzt4an=xzt4.getAnswer();
         xzt4opA=xzt4.getOptionA();
		 xzt4opB=xzt4.getOptionB();
	     xzt4opC=xzt4.getOptionC();
		 xzt4opD=xzt4.getOptionD();
		 
		 xzt5qu=xzt5.getQuestion();
	     xzt5an=xzt5.getAnswer();
         xzt5opA=xzt5.getOptionA();
		 xzt5opB=xzt5.getOptionB();
	     xzt5opC=xzt5.getOptionC();
		 xzt5opD=xzt5.getOptionD();
		 
		 xzt6qu=xzt6.getQuestion();
	     xzt6an=xzt6.getAnswer();
         xzt6opA=xzt6.getOptionA();
		 xzt6opB=xzt6.getOptionB();
	     xzt6opC=xzt6.getOptionC();
		 xzt6opD=xzt6.getOptionD();
		 
		 xzt7qu=xzt7.getQuestion();
	     xzt7an=xzt7.getAnswer();
         xzt7opA=xzt7.getOptionA();
		 xzt7opB=xzt7.getOptionB();
	     xzt7opC=xzt7.getOptionC();
		 xzt7opD=xzt7.getOptionD();
		 
		 xzt8qu=xzt8.getQuestion();
	     xzt8an=xzt8.getAnswer();
         xzt8opA=xzt8.getOptionA();
		 xzt8opB=xzt8.getOptionB();
	     xzt8opC=xzt8.getOptionC();
		 xzt8opD=xzt8.getOptionD();
		 
		 xzt9qu=xzt9.getQuestion();
	     xzt9an=xzt9.getAnswer();
         xzt9opA=xzt9.getOptionA();
		 xzt9opB=xzt9.getOptionB();
	     xzt9opC=xzt9.getOptionC();
		 xzt9opD=xzt9.getOptionD();
		 
		 xzt10qu=xzt10.getQuestion();
	     xzt10an=xzt10.getAnswer();
         xzt10opA=xzt10.getOptionA();
		 xzt10opB=xzt10.getOptionB();
	     xzt10opC=xzt10.getOptionC();
		 xzt10opD=xzt10.getOptionD();
		 
		 xzt1qu=xzt1.getQuestion();
	     xzt1an=xzt1.getAnswer();
         xzt1opA=xzt1.getOptionA();
		 xzt1opB=xzt1.getOptionB();
	     xzt1opC=xzt1.getOptionC();
		 xzt1opD=xzt1.getOptionD();
		 
		 tkt1qu=tkt1.getQuestion();
		 tkt1an=tkt1.getAnswer();
		 
		 tkt2qu=tkt2.getQuestion();
		 tkt2an=tkt2.getAnswer();
		 
		 tkt3qu=tkt3.getQuestion();
		 tkt3an=tkt3.getAnswer();
		 
		 tkt4qu=tkt4.getQuestion();
		 tkt4an=tkt4.getAnswer();
		 
		 tkt5qu=tkt5.getQuestion();
		 tkt5an=tkt5.getAnswer();
		 
		 pdt1qu=pdt1.getQuestion();
		 pdt1an=pdt1.getAnswer();
		 
		 pdt2qu=pdt2.getQuestion();
		 pdt2an=pdt2.getAnswer();
		 
		 pdt3qu=pdt3.getQuestion();
		 pdt3an=pdt3.getAnswer();
		 
		 jdt1qu=jdt1.getQuestion();
		 
		 jdt2qu=jdt2.getQuestion();

		 bct1qu=bct1.getQuestion();
		
		 bct2qu=bct2.getQuestion();

	}
	
	
	
	
	@Override
	/**
	 * 用来将试卷用字符串输出
	 * @author 李凯
	 */
	public String toString() {
		System.out.println("PaperString:");
		System.out.println("id=" + id + ", starttime=" + starttime + ", endtime=" + endtime + ", papername="+ papername);
		System.out.println("xzt1qu:" + xzt1qu);
		System.out.println("xzt1an"+xzt1an);
		System.out.println("A:"+xzt1opA);
		System.out.println("B:"+xzt1opB);
		System.out.println("C:"+xzt1opC);
		System.out.println("D:"+xzt1opD);
		
		System.out.println("xzt2qu:" + xzt2qu);
		System.out.println("xzt2an"+xzt2an);
		System.out.println("A:"+xzt1opA);
		System.out.println("B:"+xzt1opB);
		System.out.println("C:"+xzt1opC);
		System.out.println("D:"+xzt1opD);
		
		System.out.println("xzt3qu:" + xzt3qu);
		System.out.println("xzt3an"+xzt3an);
		System.out.println("A:"+xzt1opA);
		System.out.println("B:"+xzt1opB);
		System.out.println("C:"+xzt1opC);
		System.out.println("D:"+xzt1opD);
		
		System.out.println("xzt4qu:" + xzt4qu);
		System.out.println("xzt4an"+xzt4an);
		System.out.println("A:"+xzt1opA);
		System.out.println("B:"+xzt1opB);
		System.out.println("C:"+xzt1opC);
		System.out.println("D:"+xzt1opD);
		
		
		return "xzt1qu=" + xzt1qu + ", xzt1an=" + xzt1an + ", xzt1opA=" + xzt1opA + ", xzt1opB="
				+ xzt1opB + ", xzt1opC=" + xzt1opC + ", xzt1opD=" + xzt1opD + ", xzt2qu=" + xzt2qu + ", xzt2an="
				+ xzt2an + ", xzt2opA=" + xzt2opA + ", xzt2opB=" + xzt2opB + ", xzt2opC=" + xzt2opC + ", xzt2opD="
				+ xzt2opD + ", xzt3qu=" + xzt3qu + ", xzt3an=" + xzt3an + ", xzt3opA=" + xzt3opA + ", xzt3opB="
				+ xzt3opB + ", xzt3opC=" + xzt3opC + ", xzt3opD=" + xzt3opD + ", xzt4qu=" + xzt4qu + ", xzt4an="
				+ xzt4an + ", xzt4opA=" + xzt4opA + ", xzt4opB=" + xzt4opB + ", xzt4opC=" + xzt4opC + ", xzt4opD="
				+ xzt4opD + ", xzt5qu=" + xzt5qu + ", xzt5an=" + xzt5an + ", xzt5opA=" + xzt5opA + ", xzt5opB="
				+ xzt5opB + ", xzt5opC=" + xzt5opC + ", xzt5opD=" + xzt5opD + ", xzt6qu=" + xzt6qu + ", xzt6an="
				+ xzt6an + ", xzt6opA=" + xzt6opA + ", xzt6opB=" + xzt6opB + ", xzt6opC=" + xzt6opC + ", xzt6opD="
				+ xzt6opD + ", xzt7qu=" + xzt7qu + ", xzt7an=" + xzt7an + ", xzt7opA=" + xzt7opA + ", xzt7opB="
				+ xzt7opB + ", xzt7opC=" + xzt7opC + ", xzt7opD=" + xzt7opD + ", xzt8qu=" + xzt8qu + ", xzt8an="
				+ xzt8an + ", xzt8opA=" + xzt8opA + ", xzt8opB=" + xzt8opB + ", xzt8opC=" + xzt8opC + ", xzt8opD="
				+ xzt8opD + ", xzt9qu=" + xzt9qu + ", xzt9an=" + xzt9an + ", xzt9opA=" + xzt9opA + ", xzt9opB="
				+ xzt9opB + ", xzt9opC=" + xzt9opC + ", xzt9opD=" + xzt9opD + ", xzt10qu=" + xzt10qu + ", xzt10an="
				+ xzt10an + ", xzt10opA=" + xzt10opA + ", xzt10opB=" + xzt10opB + ", xzt10opC=" + xzt10opC
				+ ", xzt10opD=" + xzt10opD + ", tkt1qu=" + tkt1qu + ", tkt1an=" + tkt1an + ", tkt2qu=" + tkt2qu
				+ ", tkt2an=" + tkt2an + ", tkt3qu=" + tkt3qu + ", tkt3an=" + tkt3an + ", tkt4qu=" + tkt4qu
				+ ", tkt4an=" + tkt4an + ", tkt5qu=" + tkt5qu + ", tkt5an=" + tkt5an +"pdt1qu="+pdt1qu+"pdt1an="+pdt1an
				+"pdt2qu="+pdt2qu+"pdt2an="+ pdt2an+"pdt3qu="+pdt3qu+"pdt3an="+pdt3an+"jdt1qu="+jdt1qu+
				"jdt2qu="+jdt2qu+", bct1qu=" + bct1qu+ ", bct2qu=" + bct2qu + "]";
	}




	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public Timestamp getStarttime() {
		return starttime;
	}
	public void setStarttime(Timestamp starttime) {
		this.starttime = starttime;
	}
	public Timestamp getEndtime() {
		return endtime;
	}
	public void setEndtime(Timestamp endtime) {
		this.endtime = endtime;
	}
	public Time getTime() {
		return time;
	}
	public void setTime(Time time) {
		this.time = time;
	}
	public String getPapername() {
		return papername;
	}
	public void setPapername(String papername) {
		this.papername = papername;
	}
	public String getClass1() {
		return class1;
	}
	public void setClass1(String class1) {
		this.class1 = class1;
	}
	public String getXzt1qu() {
		return xzt1qu;
	}
	public void setXzt1qu(String xzt1qu) {
		this.xzt1qu = xzt1qu;
	}
	public String getXzt1an() {
		return xzt1an;
	}
	public void setXzt1an(String xzt1an) {
		this.xzt1an = xzt1an;
	}
	public String getXzt1opA() {
		return xzt1opA;
	}
	public void setXzt1opA(String xzt1opA) {
		this.xzt1opA = xzt1opA;
	}
	public String getXzt1opB() {
		return xzt1opB;
	}
	public void setXzt1opB(String xzt1opB) {
		this.xzt1opB = xzt1opB;
	}
	public String getXzt1opC() {
		return xzt1opC;
	}
	public void setXzt1opC(String xzt1opC) {
		this.xzt1opC = xzt1opC;
	}
	public String getXzt1opD() {
		return xzt1opD;
	}
	public void setXzt1opD(String xzt1opD) {
		this.xzt1opD = xzt1opD;
	}
	public String getXzt2qu() {
		return xzt2qu;
	}
	public void setXzt2qu(String xzt2qu) {
		this.xzt2qu = xzt2qu;
	}
	public String getXzt2an() {
		return xzt2an;
	}
	public void setXzt2an(String xzt2an) {
		this.xzt2an = xzt2an;
	}
	public String getXzt2opA() {
		return xzt2opA;
	}
	public void setXzt2opA(String xzt2opA) {
		this.xzt2opA = xzt2opA;
	}
	public String getXzt2opB() {
		return xzt2opB;
	}
	public void setXzt2opB(String xzt2opB) {
		this.xzt2opB = xzt2opB;
	}
	public String getXzt2opC() {
		return xzt2opC;
	}
	public void setXzt2opC(String xzt2opC) {
		this.xzt2opC = xzt2opC;
	}
	public String getXzt2opD() {
		return xzt2opD;
	}
	public void setXzt2opD(String xzt2opD) {
		this.xzt2opD = xzt2opD;
	}
	public String getXzt3qu() {
		return xzt3qu;
	}
	public void setXzt3qu(String xzt3qu) {
		this.xzt3qu = xzt3qu;
	}
	public String getXzt3an() {
		return xzt3an;
	}
	public void setXzt3an(String xzt3an) {
		this.xzt3an = xzt3an;
	}
	public String getXzt3opA() {
		return xzt3opA;
	}
	public void setXzt3opA(String xzt3opA) {
		this.xzt3opA = xzt3opA;
	}
	public String getXzt3opB() {
		return xzt3opB;
	}
	public void setXzt3opB(String xzt3opB) {
		this.xzt3opB = xzt3opB;
	}
	public String getXzt3opC() {
		return xzt3opC;
	}
	public void setXzt3opC(String xzt3opC) {
		this.xzt3opC = xzt3opC;
	}
	public String getXzt3opD() {
		return xzt3opD;
	}
	public void setXzt3opD(String xzt3opD) {
		this.xzt3opD = xzt3opD;
	}
	public String getXzt4qu() {
		return xzt4qu;
	}
	public void setXzt4qu(String xzt4qu) {
		this.xzt4qu = xzt4qu;
	}
	public String getXzt4an() {
		return xzt4an;
	}
	public void setXzt4an(String xzt4an) {
		this.xzt4an = xzt4an;
	}
	public String getXzt4opA() {
		return xzt4opA;
	}
	public void setXzt4opA(String xzt4opA) {
		this.xzt4opA = xzt4opA;
	}
	public String getXzt4opB() {
		return xzt4opB;
	}
	public void setXzt4opB(String xzt4opB) {
		this.xzt4opB = xzt4opB;
	}
	public String getXzt4opC() {
		return xzt4opC;
	}
	public void setXzt4opC(String xzt4opC) {
		this.xzt4opC = xzt4opC;
	}
	public String getXzt4opD() {
		return xzt4opD;
	}
	public void setXzt4opD(String xzt4opD) {
		this.xzt4opD = xzt4opD;
	}
	public String getXzt5qu() {
		return xzt5qu;
	}
	public void setXzt5qu(String xzt5qu) {
		this.xzt5qu = xzt5qu;
	}
	public String getXzt5an() {
		return xzt5an;
	}
	public void setXzt5an(String xzt5an) {
		this.xzt5an = xzt5an;
	}
	public String getXzt5opA() {
		return xzt5opA;
	}
	public void setXzt5opA(String xzt5opA) {
		this.xzt5opA = xzt5opA;
	}
	public String getXzt5opB() {
		return xzt5opB;
	}
	public void setXzt5opB(String xzt5opB) {
		this.xzt5opB = xzt5opB;
	}
	public String getXzt5opC() {
		return xzt5opC;
	}
	public void setXzt5opC(String xzt5opC) {
		this.xzt5opC = xzt5opC;
	}
	public String getXzt5opD() {
		return xzt5opD;
	}
	public void setXzt5opD(String xzt5opD) {
		this.xzt5opD = xzt5opD;
	}
	public String getXzt6qu() {
		return xzt6qu;
	}
	public void setXzt6qu(String xzt6qu) {
		this.xzt6qu = xzt6qu;
	}
	public String getXzt6an() {
		return xzt6an;
	}
	public void setXzt6an(String xzt6an) {
		this.xzt6an = xzt6an;
	}
	public String getXzt6opA() {
		return xzt6opA;
	}
	public void setXzt6opA(String xzt6opA) {
		this.xzt6opA = xzt6opA;
	}
	public String getXzt6opB() {
		return xzt6opB;
	}
	public void setXzt6opB(String xzt6opB) {
		this.xzt6opB = xzt6opB;
	}
	public String getXzt6opC() {
		return xzt6opC;
	}
	public void setXzt6opC(String xzt6opC) {
		this.xzt6opC = xzt6opC;
	}
	public String getXzt6opD() {
		return xzt6opD;
	}
	public void setXzt6opD(String xzt6opD) {
		this.xzt6opD = xzt6opD;
	}
	public String getXzt7qu() {
		return xzt7qu;
	}
	public void setXzt7qu(String xzt7qu) {
		this.xzt7qu = xzt7qu;
	}
	public String getXzt7an() {
		return xzt7an;
	}
	public void setXzt7an(String xzt7an) {
		this.xzt7an = xzt7an;
	}
	public String getXzt7opA() {
		return xzt7opA;
	}
	public void setXzt7opA(String xzt7opA) {
		this.xzt7opA = xzt7opA;
	}
	public String getXzt7opB() {
		return xzt7opB;
	}
	public void setXzt7opB(String xzt7opB) {
		this.xzt7opB = xzt7opB;
	}
	public String getXzt7opC() {
		return xzt7opC;
	}
	public void setXzt7opC(String xzt7opC) {
		this.xzt7opC = xzt7opC;
	}
	public String getXzt7opD() {
		return xzt7opD;
	}
	public void setXzt7opD(String xzt7opD) {
		this.xzt7opD = xzt7opD;
	}
	public String getXzt8qu() {
		return xzt8qu;
	}
	public void setXzt8qu(String xzt8qu) {
		this.xzt8qu = xzt8qu;
	}
	public String getXzt8an() {
		return xzt8an;
	}
	public void setXzt8an(String xzt8an) {
		this.xzt8an = xzt8an;
	}
	public String getXzt8opA() {
		return xzt8opA;
	}
	public void setXzt8opA(String xzt8opA) {
		this.xzt8opA = xzt8opA;
	}
	public String getXzt8opB() {
		return xzt8opB;
	}
	public void setXzt8opB(String xzt8opB) {
		this.xzt8opB = xzt8opB;
	}
	public String getXzt8opC() {
		return xzt8opC;
	}
	public void setXzt8opC(String xzt8opC) {
		this.xzt8opC = xzt8opC;
	}
	public String getXzt8opD() {
		return xzt8opD;
	}
	public void setXzt8opD(String xzt8opD) {
		this.xzt8opD = xzt8opD;
	}
	public String getXzt9qu() {
		return xzt9qu;
	}
	public void setXzt9qu(String xzt9qu) {
		this.xzt9qu = xzt9qu;
	}
	public String getXzt9an() {
		return xzt9an;
	}
	public void setXzt9an(String xzt9an) {
		this.xzt9an = xzt9an;
	}
	public String getXzt9opA() {
		return xzt9opA;
	}
	public void setXzt9opA(String xzt9opA) {
		this.xzt9opA = xzt9opA;
	}
	public String getXzt9opB() {
		return xzt9opB;
	}
	public void setXzt9opB(String xzt9opB) {
		this.xzt9opB = xzt9opB;
	}
	public String getXzt9opC() {
		return xzt9opC;
	}
	public void setXzt9opC(String xzt9opC) {
		this.xzt9opC = xzt9opC;
	}
	public String getXzt9opD() {
		return xzt9opD;
	}
	public void setXzt9opD(String xzt9opD) {
		this.xzt9opD = xzt9opD;
	}
	public String getXzt10qu() {
		return xzt10qu;
	}
	public void setXzt10qu(String xzt10qu) {
		this.xzt10qu = xzt10qu;
	}
	public String getXzt10an() {
		return xzt10an;
	}
	public void setXzt10an(String xzt10an) {
		this.xzt10an = xzt10an;
	}
	public String getXzt10opA() {
		return xzt10opA;
	}
	public void setXzt10opA(String xzt10opA) {
		this.xzt10opA = xzt10opA;
	}
	public String getXzt10opB() {
		return xzt10opB;
	}
	public void setXzt10opB(String xzt10opB) {
		this.xzt10opB = xzt10opB;
	}
	public String getXzt10opC() {
		return xzt10opC;
	}
	public void setXzt10opC(String xzt10opC) {
		this.xzt10opC = xzt10opC;
	}
	public String getXzt10opD() {
		return xzt10opD;
	}
	public void setXzt10opD(String xzt10opD) {
		this.xzt10opD = xzt10opD;
	}
	public String getTkt1qu() {
		return tkt1qu;
	}
	public void setTkt1qu(String tkt1qu) {
		this.tkt1qu = tkt1qu;
	}
	public String getTkt1an() {
		return tkt1an;
	}
	public void setTkt1an(String tkt1an) {
		this.tkt1an = tkt1an;
	}
	public String getTkt2qu() {
		return tkt2qu;
	}
	public void setTkt2qu(String tkt2qu) {
		this.tkt2qu = tkt2qu;
	}
	public String getTkt2an() {
		return tkt2an;
	}
	public void setTkt2an(String tkt2an) {
		this.tkt2an = tkt2an;
	}
	public String getTkt3qu() {
		return tkt3qu;
	}
	public void setTkt3qu(String tkt3qu) {
		this.tkt3qu = tkt3qu;
	}
	public String getTkt3an() {
		return tkt3an;
	}
	public void setTkt3an(String tkt3an) {
		this.tkt3an = tkt3an;
	}
	public String getTkt4qu() {
		return tkt4qu;
	}
	public void setTkt4qu(String tkt4qu) {
		this.tkt4qu = tkt4qu;
	}
	public String getTkt4an() {
		return tkt4an;
	}
	public void setTkt4an(String tkt4an) {
		this.tkt4an = tkt4an;
	}
	public String getTkt5qu() {
		return tkt5qu;
	}
	public void setTkt5qu(String tkt5qu) {
		this.tkt5qu = tkt5qu;
	}
	public String getTkt5an() {
		return tkt5an;
	}
	public void setTkt5an(String tkt5an) {
		this.tkt5an = tkt5an;
	}
	
	
	public String getPdt1qu() {
		return pdt1qu;
	}
	public void setPdt1qu(String pdt1qu) {
		this.pdt1qu = pdt1qu;
	}
	public String getPdt1an() {
		return pdt1an;
	}
	public void setPdt1an(String pdt1an) {
		this.pdt1an = pdt1an;
	}
	public String getPdt2qu() {
		return pdt2qu;
	}
	public void setPdt2qu(String pdt2qu) {
		this.pdt2qu = pdt2qu;
	}
	public String getPdt2an() {
		return pdt2an;
	}
	public void setPdt2an(String pdt2an) {
		this.pdt2an = pdt2an;
	}
	public String getPdt3qu() {
		return pdt3qu;
	}
	public void setPdt3qu(String pdt3qu) {
		this.pdt3qu = pdt3qu;
	}
	public String getPdt3an() {
		return pdt3an;
	}
	public void setPdt3an(String pdt3an) {
		this.pdt3an = pdt3an;
	}
	
	
	public String getJdt1qu() {
		return jdt1qu;
	}
	public void setJdt1qu(String jdt1qu) {
		this.jdt1qu = jdt1qu;
	}
	public String getJdt2qu() {
		return jdt1qu;
	}
	public void setJdt2qu(String jdt2qu) {
		this.jdt2qu = jdt2qu;
	}
	
	
	
	
	public String getBct1qu() {
		return bct1qu;
	}
	public void setBct1qu(String bct1qu) {
		this.bct1qu = bct1qu;
	}
	public String getBct2qu() {
		return bct2qu;
	}
	public void setBct2qu(String bct2qu) {
		this.bct2qu = bct2qu;
	}
}
