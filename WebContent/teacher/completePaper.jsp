	...
	<form name="form1" method="post" action="../ShowCorrectPaper" style="margin-top:15px">
		...
		<div>
	 			<table class="tt" border="1px solid" style="width:95%;border-color: #36AAFD">
	 				...
	 				<tr>
	 				  <td><center><font style="font-size: 24px; font-family: small arial,sans-serif">${ly.name}</font></center></td>
	 				  <!--进入选中的已批阅试卷的页面，根据选中的试卷试卷名查询数据库
    					链接值为后台存储学生上传的答案的文件-->
    					<td><center><font style="font-size: 24px; font-family: small arial,sans-serif">${ly.getUserId()}</font></center></td>
    					<td><center><font style="font-size: 24px; font-family: small arial,sans-serif">${ly.papername}</font></center></td>
    					<td><center><font style="font-size: 24px; font-family: small arial,sans-serif">${ly.getPaperId()}</font></center></td>
    					<%-- <td><button type="button" class="text-center" style="width:100%; height: 30px; 
		border: none; background-color: #36AAFD; color: white;" onclick="window.location.href='../CorrectPaper?papername=${ly.getPaperId()}&username=${ly.getUserId()}&type=1'">进入</button></td>			 --%>	  	
	 				</tr>
